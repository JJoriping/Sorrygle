...
export class Sorrygle{
  ...
  public parse():void{
    ...
    function addNote(i:number, breakChord?:boolean):void{
      ...
      if(!track) throw Error(`#${i} No channel specified`);
      if(track.transpose){
        prevNote[0] = prevNote[0].map(transpose);
        if(pendingGrace) pendingGrace = pendingGrace.map(transpose);
      }
      if(octaveOffset){
      ...
      for(const v of pendingGrace || []){
        track.position += Sorrygle.GRACE_LENGTH;
      ...
      if(pendingDiacritics.length){
        ...
        track.position += getTickDuration(options.duration);
    ...
    function transpose(v:MIDI.Pitch):MIDI.Pitch{
      const [ , a, b ] = v.match(Sorrygle.REGEXP_PITCH)!;
      const sequence = Sorrygle.NOTE_SEQUENCES.indexOf(a as any);
      let newSequence = sequence + track!.transpose;
      let carry = 0;
    ...
    function popDiacritic():void{
      ...
      diacritic.notes.forEach((v, i, my) => {
        ...
        if(pendingDiacritics.length){
          pendingDiacritics[pendingDiacritics.length - 1].notes.push(v);
        }else{
          track!.position += getTickDuration(v.duration);
          track!.data.addEvent(new MIDI.NoteEvent(v));
        }
      });
      if(internalWait.length){
